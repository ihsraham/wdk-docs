...
| --- | --- |
| [`AmountInput`](api-reference#amountinput) | Numeric input with token/fiat toggle, balance helper and Max action |
| [`AssetSelector`](api-reference#assetselector) | Token search & pick list with recent items and empty states |
| [`NetworkSelector`](api-reference#networkselector) | Network picker with gas level indicators and colors |
| [`Balance`](api-reference#balance) | Displays a balance value with optional masking and custom loader |
| [`CryptoAddressInput`](api-reference#cryptoaddressinput) | Address input with QR scan and paste helpers, validation state |
| [`QRCode`](api-reference#qrcode) | QR renderer for addresses/payment requests with labeling and styling |
| [`TransactionItem`](api-reference#transactionitem) | Single transaction row (sent/received) with token, amounts, network |
| [`TransactionList`](api-reference#transactionlist) | Virtualized list of transactions using `TransactionItem` |
| [`SeedPhrase`](api-reference#seedphrase) | Grid of seed words with optional editing and loading states |
